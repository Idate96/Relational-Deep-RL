...
if __name__ == "__main__":
    ...
    policy_kwargs = dict(
        features_extractor_class=RelationalNet,
        features_extractor_kwargs=(dict(baseline=True, num_res_blocks=1, mlp_depth=3)),
    ...
    model = PPO(
        ...
        gamma=1,
        batch_size=64,
        n_steps=256, # with 12 environments these are 32 trajectories 
        n_epochs=10,
        ent_coef=0.001,
    ...
    checkpoint_callback = CheckpointCallback(save_freq=200000, save_path=log_dir, name_prefix="ppo_resnet")
    ...
    model.learn(10000000, callback=callbacks, tb_log_name="ppo_resnet", reset_num_timesteps=False)

    # save the model
    model.save("ppo_boxworld_res_test")
